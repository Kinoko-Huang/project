...
##### Step 0 Detele previous logfiles #########
 rm -f $CAMx_HOME/runfiles/CAMx5.01.HKPATH${HH}z.27_9.*.node*
 rm -f $CAMx_HOME/runfiles/CAMx5.01.HKPATH${HH}z.3.*.node*
 rm -f $CAMx_HOME/runfiles/CAMx5.01.HKPATH${HH}z.1.*.node*
 rm -f $CAMx_HOME/runfiles/CAMx.in
 #rm -f $CAMx_HOME/runfiles/nodes
 rm -f $CAMx_HOME/bndextr/msg/camx.3km.*.diag*
...
##### Step 2 Prepare ahomap file for CAMx #########
  cd $AHOMAP/work
    csh -f $AHOMAP/work/ahomap.hk${HH}z.27.job $JULBDATE $JULEDATE 
    csh -f $AHOMAP/work/ahomap.hk${HH}z.9.job $JULBDATE $JULEDATE
    csh -f $AHOMAP/work/ahomap.hk${HH}z.3.job $JULBDATE $JULEDATE 
    csh -f $AHOMAP/work/ahomap.hk${HH}z.1.job $JULBDATE $JULEDATE 
    if ( $status != 0) then
...
##### Step 3 Prepare tuv file for CAMx ############
  ...
  cd $TUV
    csh -f tuv4.0_1.hkpath.27km.job   $JULBDATE $JULEDATE
    csh -f tuv4.0_1.hkpath.9km.job    $JULBDATE $JULEDATE
    csh -f tuv4.0_1.hkpath.3km.job    $JULBDATE $JULEDATE
    csh -f tuv4.0_1.hkpath.1km.job    $JULBDATE $JULEDATE 
    if ( $status != 0) then
