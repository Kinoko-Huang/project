...
if ( "$LINKMCIP" == 1 ) then
  ...
  foreach DMAIN ( 27km 9km 3km 1km )
    /bin/ln -sf $MCIPDIR/$YYYY/$YYYY$MM/{$YYYY}{$MM}{$DD}{$HH}/$DMAIN/GRID* $INDIR/met/$DMAIN/
    if ( $status != 0 ) then
      echo "Failed: /bin/ln -sf $MCIPDIR/$YYYY/$YYYY$MM/${YYYY}${MM}${DD}${HH}/$DMAIN/GRID* $INDIR/met/$DMAIN/"
      exit 1
    endif
    /bin/ln -sf $MCIPDIR/$YYYY/$YYYY$MM/{$YYYY}{$MM}{$DD}{$HH}/$DMAIN/MET* $INDIR/met/$DMAIN/
    if ( $status != 0 ) then
      echo "Failed: /bin/ln -sf $MCIPDIR/$YYYY/$YYYY$MM/${YYYY}${MM}${DD}${HH}/$DMAIN/MET* $INDIR/met/$DMAIN/"
      exit 1
    endif
  end
endif
...
if ( "$PICKEMIS" == 1 ) then
  ...
  while ("$IDATE" <= "$JULILAST" )
    ...
    foreach DMAIN ( 27km 9km 3km 1km )
      switch ( $IDOW )
      case Sat:
        setenv INFILE ${EGEN_DIR}/$DMAIN/merged/emiss_CB05.HongKong.${DMAIN}_Sat${hour12z}.ncf
        breaksw
      case Sun:
        setenv INFILE ${EGEN_DIR}/$DMAIN/merged/emiss_CB05.HongKong.${DMAIN}_Sun${hour12z}.ncf
        breaksw
      case Mon:
        setenv INFILE ${EGEN_DIR}/$DMAIN/merged/emiss_CB05.HongKong.${DMAIN}_Mon${hour12z}.ncf
        breaksw
      case Tue:
        setenv INFILE ${EGEN_DIR}/$DMAIN/merged/emiss_CB05.HongKong.${DMAIN}_Tue${hour12z}.ncf
        breaksw
      case Wed:
        setenv INFILE ${EGEN_DIR}/$DMAIN/merged/emiss_CB05.HongKong.${DMAIN}_Wed${hour12z}.ncf
        breaksw
      case Thu:
        setenv INFILE ${EGEN_DIR}/$DMAIN/merged/emiss_CB05.HongKong.${DMAIN}_Thu${hour12z}.ncf
        breaksw
      case Fri:
        setenv INFILE ${EGEN_DIR}/$DMAIN/merged/emiss_CB05.HongKong.${DMAIN}_Fri${hour12z}.ncf
        breaksw
      endsw
      setenv OUTFILE $INDIR/emiss/$DMAIN/emiss_CB05.HongKong.${DMAIN}_${IDATE}.ncf
      cd $UTILDIR/m3tshift
  ...
  set IDATE = $JULIBEGN
  setenv OUTFILE $INDIR/emiss/$DMAIN/emiss_CB05.HongKong.${DMAIN}_${IDATE}.ncf  #(point to our SMOKE-cmaqfc output file)
...
if ( "$PICKICBC" == 1 ) then
  ...
  if ( "$MM" == "12" | "$MM" == "01" | "$MM" == "02" ) then
    ...
    setenv ICON27_path $INDIR/icbc/27km
    setenv ICON27_file ICON_COMBINE_2006002
    ...
    setenv ICON27_path $INDIR/icbc/27km
    setenv ICON27_file ICON_COMBINE_2006091
    ...
    setenv ICON27_path $INDIR/icbc/27km
    setenv ICON27_file ICON_COMBINE_2006182
    ...
    setenv ICON27_path $INDIR/icbc/27km
    setenv ICON27_file ICON_COMBINE_2006274
...
while ( "$IDATE" <= "$JULILAST" )
  ...
  foreach DMAIN ( 27km 9km 3km 1km )  
    ...
    echo NOW IS RUNNING BCON ON $DMAIN at $IDATE ...
    # run bcon
    if ( "$DMAIN" != "27km" ) then
      cd $OPDIR/preproc/bcon
       ./run.bcon.$MIDNAME.OP.job $IDATE
      if ( $status != 0 ) then
        echo Failed: ./run.bcon.$MIDNAME.OP.job $IDATE
        exit 1
      endif
    endif
    ...
    if ( "$DMAIN" != "27km" ) then
      if ( "$IDATE" == "$JULIBEGN" ) then
        cd $OPDIR/preproc/icon
         ./run.icon.$MIDNAME.OP.job $IDATE
        if ( $status != 0 ) then
          echo Failed: ./run.icon.$MIDNAME.OP.job $IDATE
          exit 1
        endif
      endif
    endif

    echo NOW IS RUNNING CCTM ON $DMAIN at $IDATE ...
    cd $OPDIR
    # run cmaq
    cd $OPDIR/runfiles/$DMAIN
    if ( $CTM_COLD_START == 1 && "$IDATE" == "$JULIBEGN" ) then
      echo "CMAQ cold start domain = $DMAIN and date = $JULIBEGN"
      if ( "$DMAIN" == "27km" ) then
        echo Run ./run.OP.$MIDNAME.mpich2_cold $IDATE $ICON27_path $ICON27_file
         ./run.OP.$MIDNAME.mpich2_cold $IDATE $ICON27_path $ICON27_file
        if ( $status != 0 ) then
          echo Failed: ./run.OP.$MIDNAME.mpich2_cold $IDATE $ICON27_path $ICON27_file
          exit 1
        endif
      else
        echo Run ./run.OP.$MIDNAME.mpich2_cold $IDATE
        ./run.OP.$MIDNAME.mpich2_cold $IDATE
        if ( $status != 0 ) then
          echo Failed: ./run.OP.$MIDNAME.mpich2_cold $IDATE
          exit 1
        endif
      endif
    else
