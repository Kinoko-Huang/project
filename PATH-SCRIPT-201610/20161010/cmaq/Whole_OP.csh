...
set BEG_YES_JDATE = `$PATH_SYSDIR/bin/yj_prev $BEG_JDATE 1`
...
if ( "$PICKEMIS" == 1 ) then
  ...
  while ("$CUR_JDATE" <= "$END_JDATE" )
    ...
    setenv IDOW `$PATH_SYSDIR/bin/yyyymmdd2dow $CUR_GDATE`
    foreach DOMAINS_RES ( $G_DOMAINS_RES_CMAQ )
      switch ( $IDOW )
      case Sat:
  ...
  foreach DOMAINS_RES ( $G_DOMAINS_RES_CMAQ )  
   # setenv MIDNAME hk${DOMAINS_RES}
    ...
    if ( $CTM_COLD_START == 1 && "$CUR_JDATE" == "$BEG_JDATE" ) then
      ...
      if ( "$CUR_JDATE" == "$BEG_JDATE" && ! -f ${EXEC}_${M3EXTN}.CGRID.$BEG_YES_JDATE ) then
        setenv NEW_START true
      else
