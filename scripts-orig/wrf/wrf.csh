...
##--- WPS and NPS             ---##
#####+++++++++++++++++++++++++#####
setenv truelat1 15.0
setenv truelat2 40.0
setenv ref_LAT 28.5
setenv ref_LON 114.0
setenv dx 27
...
setenv year_start `echo ${date_start} | cut -c 1-4`
setenv month_start `echo ${date_start} | cut -c 5-6`
setenv day_start `echo ${date_start} | cut -c 7-8`
setenv hour_start `echo ${date_start} | cut -c 9-10`
setenv year_end `echo ${date_end} | cut -c 1-4`
setenv month_end `echo ${date_end} | cut -c 5-6`
setenv day_end `echo ${date_end} | cut -c 7-8` 
setenv hour_end `echo ${date_end} | cut -c 9-10`
...
# revised by Bo Xie on 04/12/2012
    cd ${WPS_root}
      #setenv filename_obs /disk/dataop_scratch/NUDGING/obsgrid/${year_start}${month_start}_d03/obs
      setenv filename_obs ${work_root}/obsfiles/obs
# process domain 3 nudging file
      ...
      foreach file (`ls -l ${WPS_root}/OBS_DOMAIN3* | awk '{print $9}'`)
        cat $file >> ${obs_root}/OBS_DOMAIN301_${year_start}${month_start}${day_start}
      end
# process domain 4 nudging file
      ...
      foreach file (`ls -l ${WPS_root}/OBS_DOMAIN4* | awk '{print $9}'`)
        cat $file >> ${obs_root}/OBS_DOMAIN401_${year_start}${month_start}${day_start}
      end

      unsetenv filename_obs
      unsetenv id_grid
...
echo '#########################'
    ...
    #if ( $FC_MODE == 1 ) then
    cd ${WPS_root}
      setenv filename_obs ${work_root}/gts_decoder/derived_data/obs
      setenv id_grid 1
      ./run_obsgrid.csh
      if ( $status != 0 ) then
        echo "Failed to run ./run_obsgrid.csh"
...
#   Emit for the fix of problematic forcast run 
    if ( $FC_MODE == 0 ) then    # Historical run
      echo cp -f ${obs_root}/OBS_DOMAIN301_${year_start}${month_start}${day_start} ${WRF_root}/OBS_DOMAIN301
      cp -f ${obs_root}/OBS_DOMAIN301_${year_start}${month_start}${day_start} ${WRF_root}/OBS_DOMAIN301
      echo cp -f ${obs_root}/OBS_DOMAIN401_${year_start}${month_start}${day_start} ${WRF_root}/OBS_DOMAIN401
      cp -f ${obs_root}/OBS_DOMAIN401_${year_start}${month_start}${day_start} ${WRF_root}/OBS_DOMAIN401
    endif
    if ( $FC_MODE == 0 ) then
    ...
    mv -f rsl.* wrf_log/
    set archive_folder = ${archive_root}/${year_start}/${year_start}${month_start}/${year_start}${month_start}${day_start}${hour_start}
